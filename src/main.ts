...
const updateDates = () => {
    ...
    for (const divElement of divElements) {
        (divElement.querySelector(".person-age") as HTMLElement).textContent = getReadableAge(divElement.dataset.birthDate!, now);
        (divElement.querySelector(".person-actual-age") as HTMLElement).textContent = getActualAge(divElement.dataset.birthDate!);
        (divElement.querySelector(".person-next-birthday") as HTMLElement).textContent = `${getReadableNextBirthday(divElement.dataset.birthDate!, now)} (en ${getReadableTimeDifference(getNextBirthday(divElement.dataset.birthDate!, now).getTime() - now.getTime())})`;
    }
};
